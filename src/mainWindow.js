...
try {
	...
	for (const key of Object.keys(console)) {
		if (console[key].__sentry_original__) {
...
const themesync = async () => {
	...
	const value = `body { ${vars.reduce((acc, x) => {
		const newAcc = `${acc}${x}: ${getVar(x)}; `;
		return newAcc;
	}, "")} }`;
...
setInterval(() => {
	const host = [
		...document.querySelectorAll(
			'[class*="sidebar"] [class*="info"] [class*="line"]',
		),
	...
	const oaHash = document
		.querySelector('[class*="versionHash"]')
		.cloneNode(true);
	...
	if (!advanced) {
		advanced = document.querySelector(
			':has([class^=socialLinks]) > [class^="header"] + [class^="item"] + [class^="item"] + [class^="item"] + [class^="item"] + [class^="item"] + [class^="item"] + [class^="item"] + [class^="item"] + [class^="item"] + [class^="item"]:has(+ [class^="separator"])',
		);
	}
	...
	if (!advanced) {
		advanced = document.querySelector(
			':has([class^=socialLinks]) > [class^="item"]:has(+ [class^="separator"] + :nth-last-child(1 of [class^=header]))',
		);
	}
	...
	if (!advanced) {
		advanced = [...document.querySelectorAll('[class^="item"]')].find(
			(x) => x.textContent === "Advanced",
		);
	}
