...
const { readFileSync, existsSync, renameSync } = require("node:fs");
...
const startCore = () => {
	if (oaConfig.js || oaConfig.css)
		session.defaultSession.webRequest.onHeadersReceived((d, cb) => {
			d.responseHeaders["content-security-policy"] = undefined;
			cb(d);
		});
...
module.exports = () => {
	...
	if (
		...
		!(
			process.argv?.includes?.("--multi-instance") ||
			oaConfig.multiInstance === true
		)
	)
		return app.quit();
