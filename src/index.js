...
global.log = (area, ...args) =>
	console.log(`[\x1b[38;2;88;101;242mWumpAsar\x1b[0m > ${area}]`, ...args); // Make log global for easy usage everywhere

global.oaVersion = "nightly";

log("Init", "WumpAsar", oaVersion);

if (process.resourcesPath.startsWith("/usr/lib/electron"))
	global.systemElectron = true; // Using system electron, flag for other places
...
const M = require("node:module"); // Module

const b = join(paths.getExeDir(), "modules"); // Base dir
if (process.platform === "win32")
	try {
		for (const m of require("node:fs").readdirSync(b))
...
M._resolveLookupPaths = (request, parent) => {
	if (parent?.paths?.length > 0)
		parent.paths = parent.paths.concat(M.globalPaths);
