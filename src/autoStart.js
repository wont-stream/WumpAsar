...
const desktopPath = join(
	app.getPath("appData"),
	"autostart",
	`${app.name}-${buildInfo.releaseChannel}.desktop`,
);

const exec = app.getPath("exe");

// Windows registry util
const reg = (a, c) => require("node:child_process").execFile("reg.exe", a, c);
		...
		case "win32":
			...
			reg(
				[
					...
					...queuePrefix,
					"/d",
					`"${join(exec, "..", "..", "Update.exe")}" --processStart ${basename(exec)}${
...
exports.update = (cb) =>
	...
	exports.isInstalled((installed) => (installed ? exports.install(cb) : cb()));
