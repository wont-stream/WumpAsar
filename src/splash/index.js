...
const launchSplash = (startMin) => {
	...
	if (process.platform !== "darwin")
		win.on("closed", () => !launched && app.quit());
...
class UIProgress {
	// Generic class to track updating and sent states to splash
	constructor(st) {
		this.st = st;
	...
	send() {
		if (
			(toSend === -1 &&
				...
				this.progress.size > this.done.size) ||
			toSend === this.st
		) {
			const progress = Math.min(
				100,
				([...this.progress.values()].reduce((a, x) => a + x[0], 0) /
					[...this.progress.values()].reduce((a, x) => a + x[1], 0)) *
					100,
...
const initNew = async (inst) => {
	...
	while (true) {
		...
		try {
			...
			await inst.updateToLatestWithOptions(
				retryOptions,
				({ task, state, percent }) => {
					...
					const simpleRecord = (tracker, x) =>
						tracker.record(x.package_sha256, state, percent);
...
const initOld = () => {
	...
	on("downloaded", ({ failed }) => {
		// Downloaded all modules
		toSend = 1;
