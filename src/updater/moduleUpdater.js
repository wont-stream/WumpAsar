...
exports.init = (endpoint, { releaseChannel, version }) => {
	...
	try {
		...
		for (const m of ["desktop_core", "utils", "voice"]) {
			// Ignore actual bootstrap manifest and choose our own core set, others are deferred
	...
	host =
		process.platform === "linux"
			? new (class HostLinux extends require("node:events").EventEmitter {
					setFeedURL(url) {
						this.url = url;
					}
	...
	host.on("update-progress", (progress) =>
	...
	host.setFeedURL(
		`${endpoint}/updates/${releaseChannel}?platform=${platform}&version=${version}`,
	);
...
const downloadModule = async (name, ver) => {
	...
	const total = Number.parseInt(res.headers["content-length"] ?? 1, 10);
...
const installModule = async (name, ver, path) => {
	...
	execFile("unzip", ["-l", path], (e, o) => {
		const match = o.toString().match(/([0-9]+) files/);
		total = Number.parseInt(match?.[1] ?? 0);
	}); // Get total count and extract in parallel
...
exports.isInstalled = (n, v) =>
	installed[n] && !(v && installed[n].installedVersion !== v);
