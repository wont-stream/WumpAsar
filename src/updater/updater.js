...
class Updater extends require("node:events").EventEmitter {
	constructor(options) {
		super();
...
module.exports = {
	...
	tryInitUpdater: (buildInfo, repository_url) => {
		...
		const updaterContents = require("node:fs").readFileSync(
			updaterPath,
			"utf8",
		);
		if (
			updaterContents.includes("Determined this is an architecture transition")
		)
			updaterVersion = 2;
	...
	getUpdater: () => (instance?.valid && instance) || null,
};
